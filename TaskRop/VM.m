...
#define VM_MIN_KERNEL_ADDRESS  ((vm_offset_t) 0xfffffff000000000ULL)
#endif
...
static uint64_t vm_map_get_header(uint64_t vm_map_ptr)
{
    return vm_map_ptr + rc_off_vm_map_hdr;
}

static uint64_t vm_map_header_get_first_entry(uint64_t vm_header_ptr)
{
    return kread_ptr(vm_header_ptr + rc_off_vm_map_header_links_next);
}

static uint64_t vm_map_entry_get_next_entry(uint64_t vm_entry_ptr)
{
    return kread_ptr(vm_entry_ptr + rc_off_vm_map_entry_links_next);
}

static uint32_t vm_header_get_nentries(uint64_t vm_header_ptr)
{
    return kread32(vm_header_ptr + rc_off_vm_map_header_nentries);
}

static void vm_entry_get_range(uint64_t vm_entry_ptr, uint64_t *start_out, uint64_t *end_out)
{
    uint64_t range[2];
    kreadbuf(vm_entry_ptr + 0x10, &range[0], sizeof(range));
    if (start_out) *start_out = range[0];
    if (end_out)   *end_out   = range[1];
}

static void vm_map_iterate_entries(uint64_t vm_map_ptr,
                                   void (^itBlock)(uint64_t start, uint64_t end, uint64_t entry, BOOL *stop))
{
    ...
    uint32_t numEntries = vm_header_get_nentries(header);
...
static uint64_t vm_map_find_entry(uint64_t vm_map_ptr, uint64_t address)
{
...
static BOOL VM_PACKING_IS_BASE_RELATIVE(struct VmPackingParams *p)
{
    return (p->vmpp_bits + p->vmpp_shift) <= VM_KERNEL_POINTER_SIGNIFICANT_BITS;
}

static uint64_t vm_unpack_pointer(uint64_t packed, struct VmPackingParams *params)
{
...
static uint64_t vm_pack_pointer(uint64_t ptr, struct VmPackingParams *params)
{
    if (!params->vmpp_base_relative) {
...
static uint64_t VME_OFFSET(uint64_t vme_offset_raw)
{
    return vme_offset_raw << 12;
}

static struct VMObject vm_get_object(uint64_t map, uint64_t address)
{
...
static struct VMShmem vm_create_shmem_with_object(struct VMObject *object)
{
    struct VMShmem shmem = {0};

    uint64_t size        = kread64(object->address + rc_off_vm_object_vo_un1_vou_size);
    ...
    uint64_t shmemNamedEntry = task_get_ipc_port_kobject(task_self_kptr(), memoryObject);
    uint64_t shmemVMCopyAddr = kread64(shmemNamedEntry + rc_off_vm_named_entry_backing_copy);
    uint64_t nextAddr        = kread64(shmemVMCopyAddr + rc_off_vm_named_entry_size);
    ...
    uint32_t refCount = kread32(object->address + rc_off_vm_object_ref_count);
    refCount++;
    kwrite32(object->address + rc_off_vm_object_ref_count, refCount);
